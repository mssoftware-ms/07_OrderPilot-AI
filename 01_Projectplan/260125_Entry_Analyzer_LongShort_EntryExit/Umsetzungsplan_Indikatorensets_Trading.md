...
Stand: 2026-01-25 (aktualisiert mit Nutzerhinweisen)  
Scope: Neuschreibung der Tabs 4–6 im Entry Analyzer (Indicator Setup, Indicator Optimization, Indicator Result) inklusive Risikomanagement-Integration und Backtesting nach Vorgaben aus `Projektziel.md`, `Indikatorensets` und den Regime-JSON-Templates.
...
- Hinweis Nutzer: Tabs 4–6 dürfen auch komplett gelöscht und neu aufgebaut werden (Neuschreibung statt inkrementell).
...
- JSON-Governance: Schema-First, `schema_version` Pflicht. Für Struktur an **Regime-Templates** orientieren: `03_JSON/Entry_Analyzer/Regime/JSON Template/JSON Template Rules Regime.md` und `v2_schema_reference.json`. Ordnerziel für neue Indicator-Schemas: `03_JSON/Trading_Indicatorsets` (analog aufbauen, da bestehende „json v2“ evtl. nicht passend).
...
- Tab 5 Indicator Optimization
  ...
  - Datenquelle auswählbar via Radio-Buttons: **Live Chartdaten** (Standard) oder **Historical (SQLite)** mit Default-DB `data/orderpilot_historical.db` (BTCUSDT 1m, 365 Tage).
...
- Schablonen-Ableitung: Schema-Struktur (Pflichtfelder, Params mit `range`) an Regime-v2 Schema anlehnen (`v2_schema_reference.json`), ggf. eigenes Indicator-Schema als „v2.0.0“ definieren und in `03_JSON/Trading_Indicatorsets` ablegen.

## 6) Funktionale Arbeitspakete (Akzeptanzkriterien)
1. **AP1 Anforderungen/Daten klären**  
   - JSON-Schema-Entwurf für Input/Output fertig, abgestimmt; Pfade bestätigt; Orientierung an Regime-Templates; neues Zielverzeichnis `03_JSON/Trading_Indicatorsets`.
...
3. Parameter-Generation: Utility, das aus Tabelle Range-Kombinationen erzeugt (mit Max-Kombinationen-Limit + Warnung; Limit initial 1000 Durchläufe laut Nutzer).
4. Optimierungs-Worker: echte Evaluationspipeline (Indicator-Berechnung, Signal-Logik aus `Indikatorensets`, RiskManager, Backtest pro Signal-Typ) mit **Datenquellenwahl (Live vs Historical SQLite)**; bei Historical: Loader für `data/orderpilot_historical.db` (BTCUSDT 1m, 365d). Parallelisierung prüfen (Threads/async) ohne Event-Loop-Block.
...
- Backtest: deterministische Seeds, Vergleich mit Kontroll-CSV/JSON, Toleranzen für Metriken definieren; Live-Chartdaten stehen bereit (lt. Nutzer), optional Snapshot aus Chartfenster für reproduzierbare Tests sichern.
