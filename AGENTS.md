# OrderPilot-AI – Projektkontext für OpenAI Codex CLI

Dieses Dokument definiert, wie du (Codex CLI) in diesem Repository arbeiten sollst:
...
## Wie Codex CLI diese Datei nutzt

- Lege diese Datei als `AGENTS.md` ins **Repo-Root**.
- Codex sammelt Agent-Anweisungen **vom Repo-Root bis zu deinem aktuellen Arbeitsverzeichnis**.
- Pro Verzeichnis wird höchstens **eine** Datei berücksichtigt (Priorität: `AGENTS.override.md` vor `AGENTS.md`).
- Die Dateien werden **von oben nach unten zusammengefügt**; spätere (nähere) Dateien überschreiben frühere Guidance.

**Konsequenz für dich:** Wenn ein Untermodul abweichende Regeln braucht, lege dort ein zusätzliches `AGENTS.md` oder gezielt `AGENTS.override.md` ab.
...
## 9. Arbeitsweise von Codex in diesem Repo
...
**Codex-spezifisch (CLI-Modus):**
- Wenn du Dateien ändern willst, arbeite in **kleinen, klaren Schritten** und nenne **vorab** deinen Plan.
- Gib mir nach jeder Änderung:
  - **welche Dateien** du geändert hast und **warum**
  - **welche Commands** ich ausführen soll (Tests/Lint/Run) um es zu verifizieren
  - **welche Logs** du bei Fehlschlag brauchst
- Vermeide große „alles auf einmal“-Umbauten. Wenn es nicht trivial ist: erst Patch 1, dann Patch 2.

**Wichtig:** Halte dich an das Runbook `docs/ai/change-workflow.md` und fordere bei Bedarf das Template `docs/ai/context-packet-template.md` an.
...
Wenn du erkennst, dass du wiederholt denselben Fehler machst (z. B. einen bestimmten Alpaca-Endpoint falsch verwendest), ergänze die relevanten Hinweise in dieser `AGENTS.md`.
