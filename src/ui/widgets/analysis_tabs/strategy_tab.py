...
class StrategyTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.regime_label.setProperty("class", "header")
        ...
        self.gating_label.setProperty("class", "status-badge")
        self.gating_label.setProperty("state", "neutral")
        top_layout.addWidget(self.gating_label)

        # 4. Description
        self.desc_label = QLabel()
        self.desc_label.setWordWrap(True)
        self.desc_label.setProperty("class", "info-label")
        ...
        self.auto_config_btn.setProperty("class", "primary")
    ...
    def _setup_ai_analysis_section(self, splitter: QSplitter):
        ...
        self.btn_analyze_trend.setProperty("class", "info")
        ...
        lbl_analysis.setProperty("class", "label-bold")
        ...
        self.lbl_notes.setProperty("class", "info-label")
    ...
    def _update_gating(self):
        ...
        def set_state(state):
            self.gating_label.setProperty("state", state)
            # Force style update
            self.gating_label.style().unpolish(self.gating_label)
            self.gating_label.style().polish(self.gating_label)
    ...
    def _on_analysis_finished(self, result):
        ...
        if not result:
            ...
            # For dynamic color, we might still need setStyleSheet if simple state classes aren't enough
            # But here we can use state="danger" on a label if we defined label states
            # We defined states for .status-badge. lbl_daily_bias is .label-bold.
            # I'll just leave it or use inline style for text color only.
            self.lbl_daily_bias.setStyleSheet("color: #F44336;")
            return
    ...
    def _update_analysis_display(self, result: dict):
        ...
        self.lbl_daily_bias.setStyleSheet(f"color: {bias_color};")

        # Confidence
        self.lbl_confidence.setText(f"Konfidenz: {confidence}%")
        # Keep inline color for dynamic values as it's data-driven
        conf_color = "#4CAF50" if confidence >= 70 else "#FFC107" if confidence >= 50 else "#F44336"
        self.lbl_confidence.setStyleSheet(f"color: {conf_color};")
