"""Alpaca-only Streaming Mixin for EmbeddedTradingViewChart.

CRITICAL: This is ONLY for Alpaca (Stock + Crypto).
...
class AlpacaStreamingMixin:
    ...
    def _is_valid_tick(self, price: float, reference_price: float | None) -> bool:
        """
        Prüfe ob Tick-Preis plausibel ist (Bad Tick Filter) - ALPACA ONLY.

        Alpaca kann fehlerhafte Ticks haben, daher 5% Abweichungsfilter.
        ...
        if deviation_pct > max_deviation_pct:
            logger.warning(
                f"❌ ALPACA Bad tick filtered: {self.current_symbol} price={price:.2f} "
    ...
    def _on_market_tick(self, event: Event):
        """Handle market tick event - update current candle in real-time (ALPACA ONLY)."""
        try:
            ...
            # ALPACA: Always apply 5% deviation filter
            ...
            logger.error(f"Error handling Alpaca market tick: {e}", exc_info=True)
    ...
    def _update_candle_for_tick(self, current_minute_start: int, price: float) -> None:
        if current_minute_start > self._current_candle_time:
            ...
            if hasattr(self, 'candle_closed'):
                ...
                logger.info(
                    f"🕯️ Alpaca candle closed: O={prev_open:.2f} H={prev_high:.2f} "
    ...
    def _emit_tick_price_updated(self, price: float) -> None:
        if hasattr(self, 'tick_price_updated'):
            ...
            if self._tick_emit_count % 100 == 1:
                logger.info(f"📡 Alpaca tick_price_updated emitted #{self._tick_emit_count}: {price:.2f}")
    ...
    def _process_pending_updates(self):
        ...
        try:
            ...
            logger.error(f"Error processing Alpaca updates: {e}", exc_info=True)

    def _toggle_live_stream(self):
        """Toggle live streaming on/off."""
        is_checked = self.live_stream_button.isChecked()
        logger.info(f"🔄 Alpaca live stream toggle: button_checked={is_checked}, current_enabled={self.live_streaming_enabled}")

        self.live_streaming_enabled = is_checked

        if self.live_streaming_enabled:
            logger.info(f"Starting Alpaca live stream for {self.current_symbol}...")
            asyncio.ensure_future(self._start_live_stream_async())
        else:
            logger.info("Stopping Alpaca live stream...")
    ...
    async def _start_live_stream_async(self):
        ...
        try:
            ...
            logger.error(f"Failed to start Alpaca live stream: {e}")
    ...
    async def _stop_live_stream_async(self):
        ...
        try:
            ...
            logger.error(f"Failed to stop Alpaca live stream: {e}")

    async def _start_live_stream(self):
        """Start Alpaca live streaming for current symbol."""
        ...
        try:
            # ALPACA ONLY: Detect if crypto or stock
            is_crypto = "/" in self.current_symbol

            logger.warning(
                f"🔍 START ALPACA STREAM: symbol={self.current_symbol}, crypto={is_crypto}"
            )
            ...
            if is_crypto:
                ...
                logger.info(f"✓ Alpaca crypto stream started for {self.current_symbol}")
                asset_type = "Alpaca Crypto"
            else:
                logger.warning(f"📡 Starting ALPACA STOCK stream for {self.current_symbol}")
                success = await self.history_manager.start_realtime_stream([self.current_symbol])
                logger.info(f"✓ Alpaca stock stream started for {self.current_symbol}")
                asset_type = "Alpaca Stock"
            ...
            if success:
                ...
                logger.error("Failed to start Alpaca live stream")
            ...
            logger.error(f"Error starting Alpaca live stream: {e}")
    ...
    async def _stop_live_stream(self):
        """Stop Alpaca live streaming - disconnect WebSocket."""
        if not self.history_manager:
            return
        ...
        try:
            ...
            if is_crypto:
                ...
                if hasattr(self.history_manager, 'crypto_stream_client') and self.history_manager.crypto_stream_client:
                    ...
                    logger.info(f"✓ Disconnected Alpaca crypto stream")
            else:
                # Disconnect stock stream
                if hasattr(self.history_manager, 'stream_client') and self.history_manager.stream_client:
                    ...
                    logger.info(f"✓ Disconnected Alpaca stock stream")

            self.market_status_label.setText("Ready")
            self.market_status_label.setStyleSheet("color: #888; font-weight: bold; padding: 5px;")

        except Exception as e:
            logger.error(f"Error disconnecting Alpaca stream: {e}")
