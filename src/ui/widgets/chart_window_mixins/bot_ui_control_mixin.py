...
class BotUIControlMixin:
    ...
    def _create_bot_control_tab(self) -> QWidget:
        ...
        # Issue #16: Auto-load saved defaults on startup
        self._auto_load_saved_defaults()
    ...
    def _auto_load_saved_defaults(self) -> None:
        """Issue #16: Auto-load saved defaults on startup if file exists.

        Silently loads saved settings without showing a dialog.
        """
        ...
        try:
            ...
            if default_file.exists():
                ...
                self._settings_helper.apply_bot_settings(settings)
                logger.info(f"Auto-loaded bot defaults from {default_file}")
            else:
                logger.debug("No saved bot defaults found, using widget defaults")
        except Exception as e:
            logger.warning(f"Failed to auto-load bot defaults: {e}")
